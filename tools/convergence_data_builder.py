...
def build_grid_convergence_figure(participants, case_id: str, plot_spec: dict[str, Any]) -> tuple[go.Figure, int, list[str]]:
    fig = go.Figure()
    trace_count = 0
        ...
        for zone_name, zone in case_data.grid_convergence_data.zones.items():
            ...
            if is_diameter_plot:
                ...
                for diameter, diameter_data in working_data.groupby(diameter_column):
                    ...
                    fig.add_trace(go.Scatter(x=diameter_data[x_column], y=diameter_data[y_column], mode="lines+markers", name=trace_name, legendgroup=trace_name, hovertemplate=(f"Participant: {escape(label)}<br>" f"Case: {escape(case_id)}<br>" f"Zone: {escape(zone_name)}<br>" f"Diameter={diameter:g} um<br>" f"{escape(x_column)}=%{{x}}<br>" f"{escape(y_column)}=%{{y}}<extra></extra>")))
                    trace_count += 1
